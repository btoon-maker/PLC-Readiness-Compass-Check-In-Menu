...
function initIndividualPage() {
  ...
  // Deep link support: check-in.html?checkin=vision
...
function checkinLink(checkinId) {
  ...
  // On-screen button. Hidden when printing (see .btn-link in print CSS);
  // a print-only text label is rendered alongside it by checkinPrintLine().
  return `<a class="btn-link" href="check-in.html?checkin=${c.id}">${label} &rarr;</a>`;
}

function checkinTitles(ids) {
  return ids.map(id => (CHECKINS[id] ? CHECKINS[id].title : id));
}

// Print-only line listing the recommended check-in names (buttons don't print).
function checkinPrintLine(ids) {
  if (!ids || !ids.length) return "";
  return `<p class="print-only" style="margin:.45rem 0 0;"><strong>Recommended:</strong> ${checkinTitles(ids).join(" · ")}</p>`;
}
...
function initPlannerPage() {
  ...
  const exportRow = document.getElementById("plannerExportRow");
  const exportBtn = document.getElementById("exportPlannerPdf");
  if (exportBtn) {
    exportBtn.addEventListener("click", () => exportPdf("PLC Year Check-In Planner"));
  }
  ...
  btn.addEventListener("click", () => {
    ...
    if (contextKey && CONTEXT_MAP[contextKey]) {
      ...
      html += `
        <div class="rec-card">
          ...
          ${checkinPrintLine(rec.checks)}
          ${altLinks ? `<p class="small" style="margin-top:.75rem;">Lighter alternative:</p><div>${altLinks}</div>${checkinPrintLine(rec.alt)}` : ""}
        </div>
      `;
    } else {
      html += `
    ...
    if (startVal) {
      ...
      const items = windows.map(w => {
        const buttons = w.ids.map(checkinLink).join(" ");
        return `
          <li>
